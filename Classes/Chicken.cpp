...
Chicken::Chicken(GameScene * gameScene)
{
	...
	_sprite = CCSprite::create("small_chicken.png", CCRectMake(0, 0, 25, 38));
	_sprite->setScale((windowSize.height / 20) / _sprite->getContentSize().height);
	_currentPosition.x = windowSize.width / 2;
	_currentPosition.y = (_sprite->getContentSize().height / 2) - 5; //this '5' should be a scaled number
